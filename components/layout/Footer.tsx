...
export default function Footer({ name, role, location, headline, description, links }: FooterProps) {
  return (
        ...
        <div className="contact-panel">
          <div className="contact-panel__copy">
            ...
            <div className="contact-panel__actions">
              <Button label="Email me" href="mailto:abuhager360@gmail.com" variant="inverse" />
              <Button label="Download CV" href="/resume.pdf" variant="secondary" download />
            </div>
          </div>
          <address className="contact-links">
            {links.map((link) => (
              <a key={link.href} href={link.href} {...(link.external ? { target: '_blank', rel: 'noopener noreferrer' } : {})}>
                <span className="contact-links__icon"><Icon name={link.icon} size={18} /></span>
